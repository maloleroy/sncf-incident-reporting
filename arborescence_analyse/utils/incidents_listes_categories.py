...
def excel_to_json_categories(excel_path, json_path=None):
    ...
    # Générer le dictionnaire imbriqué avec les valeurs uniques
    result = {}
    for col in df.columns:
        uniques = df[col].dropna().astype(str).unique()
        result[col] = {val: None for val in uniques}

    # Sauvegarde du JSON si un chemin est spécifié
    if json_path:
...
json_data = excel_to_json_categories("../Arbo SIGNALEMENT et motifs MGC.xlsx", "../incidents_categories.json")
