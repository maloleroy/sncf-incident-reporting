...
def find_incident(db, train, voiture, transcription) -> IncidentAnalysisResponse:
    ...
    cached = get_cached_response(transcription, possibilities)
    if cached:
        logger.info("LLM Response (from cache): %s", cached)
        response = cached["content"]
    else:
        response = get_response(transcription, possibilities)["content"]
        set_cached_response(transcription, possibilities, {"content": response})
